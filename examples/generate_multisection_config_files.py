...
SECTION1_NAME = 'configs'
SECTION2_NAME = 'section2'

# build a config permutator using imsciutils
config_factory = iu.util.ConfigFactory(**SECTION2)

# iterating through all config permutations
index = 1
for _,configs in config_factory:
    ...
    iu.info(config_factory)
    # saving the config file
    with open(filename,'w') as f:
        file_str = yaml.dump({SECTION1_NAME:configs}, default_flow_style=False)
        f.write(file_str)
